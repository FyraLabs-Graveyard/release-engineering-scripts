    ...
    if file.endswith('.spec'):
        ...
        with open('umpkg.cfg', 'w') as configfile:
            try:
                ...
                print(e)
                pass
            config.set('umpkg', 'git_repo' , f'https://gitlab.ultramarine-linux.org/dist-pkgs/{name}.git')
            # write the config file as umpkg.cfg
            config.write(configfile)
        print('Creating git repo')
        os.system('git init --initial-branch=main')
        os.system('git add .')
        os.system('git commit -m "Automated project split from RPM monorepo by batchcreate.py"')
